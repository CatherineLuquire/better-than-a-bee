...
def main():
    if flask.request.method == 'POST':
       ...
       # # brain_model is saved from
        ...
        HPLR_y = int(HPy_scaler.inverse_transform(
            HP_LRModel.predict(HPX_scaled))[0][0])
        HPE_y = int(HPy_scaler.inverse_transform(
            HP_Elastic.predict(HPX_scaled))[0])
        HPL_y = int(HPy_scaler.inverse_transform(
            HP_Lasso.predict(HPX_scaled))[0])
        HPR_y = int(HPy_scaler.inverse_transform(
            HP_Ridge.predict(HPX_scaled))[0][0])
        PVLR_y = int(PVy_scaler.inverse_transform(
            PV_LRModel.predict(PVX_scaled))[0][0])
        PVE_y = int(PVy_scaler.inverse_transform(
            PV_Elastic.predict(PVX_scaled))[0])
        PVL_y = int(PVy_scaler.inverse_transform(
            PV_Lasso.predict(PVX_scaled))[0])
        PVR_y = int(PVy_scaler.inverse_transform(
            PV_Ridge.predict(PVX_scaled))[0][0])
